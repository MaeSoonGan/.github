...
* **채널계 (AWS EKS)**: 회원·시세·주문 접수·자산 조회·랭킹·알림·WebSocket을 MSA POD로 분리하여 배치했습니다. 외부 트래픽은 Route 53 → WAF → ALB를 거쳐 들어오며, HPA를 통해 장 개시 트래픽에 맞춰 자동 확장됩니다.
* **계정계·체결 (온프레미스)**: 잔고·계좌·보유종목·체결 결과를 보관하는 **원장 DB**, 그리고 주문을 매칭하는 **체결 엔진**을 온프레미스에 배치했습니다. 가용성을 위해 DC1(Active) / DC2(Standby)를 Keepalived VIP로 구성하고, MySQL Master·Slave 복제를 통해 데이터 정합성을 확보했습니다.
* **연동**: 클라우드와 온프레미스 사이의 **비동기 이벤트는 온프레미스 Kafka**(KRaft 모드)로, **동기 조회는 VPN을 경유한 HTTP**로 처리합니다. 모든 EKS POD는 VPN을 통해 온프레미스 Kafka 및 원장 DB에 접근합니다.
* **모니터링·알림**: CloudWatch와 Prometheus/Grafana를 통합하여 클라우드·온프레 양쪽 지표를 단일 화면에서 관측하고, 체결 완료 이벤트는 Kafka → Lambda → SNS/SES를 통해 사용자에게 알림으로 전달됩니다.
...
증권사 표준 모델을 따라 시스템을 **채널계 / 계정계 / 체결계**로 분리하고, 각 계가 명확한 책임만 갖도록 설계했습니다.

* **채널계**는 고객 요청을 받아 인증·검증한 뒤, 쓰기 작업은 Kafka 이벤트로 발행하고 읽기 작업은 **스냅샷 테이블**에서만 조회합니다. 원장 DB에 직접 접근하지 않아 읽기/쓰기 책임이 분리됩니다.
* **체결계**는 Order Book을 기반으로 주문을 매칭하고, 체결 결과를 원장에 반영한 뒤 다시 이벤트로 발행합니다.
* **계정계(원장)**는 잔고·보유종목·체결 내역의 **단일 진실 공급원(Single Source of Truth)** 역할을 하며, 최종 잔고 차감과 정합성 보장을 책임집니다.

이 구조 덕분에 트래픽이 몰리는 채널계는 자유롭게 수평 확장하면서도, 정합성이 핵심인 원장은 온프레미스에서 안정적으로 운영할 수 있습니다.
...
* **AI 음성 서비스**: 음성으로 종목/가격을 질의하면 STT → LLM 의도 파악 → 한국투자증권 API 조회 → TTS로 응답 (5초 이내).
* **관측성**: OpenTelemetry로 `주문 API → Kafka → 체결 엔진 → 원장 DB → WebSocket` 전 구간을 단일 TraceID로 추적하고, 구간별 latency 병목을 시각화.
* **카오스 엔지니어링**: DC1 장애 주입, Pod 강제 종료, VPN 터널 단절 시나리오로 자동 전환(RTO)과 데이터 정합성 검증.
* **IaC / CI·CD**: Terraform으로 AWS 인프라 코드화, GitHub Actions(클라우드) / GitLab+Jenkins+Harbor(온프레) 파이프라인, ArgoCD GitOps 도입.
