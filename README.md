...
채널계는 AWS EKS, 계정계와 체결계는 온프레미스에 분리해 확장성과 데이터 정합성을 함께 확보했습니다.
02. Kafka 기반 비동기 주문·체결 처리
주문·취소 요청과 체결 결과를 Kafka 이벤트로 전달해 시스템 간 결합도를 낮추고 안정적으로 처리합니다.
03. CQRS 기반 스냅샷 조회 모델
원장은 자산 변경을 담당하고, 채널계는 동기화된 회원·주문·포트폴리오 스냅샷을 조회해 원장 부하를 분리합니다.
04. 실시간 시세 및 사용자 이벤트 스트리밍
한국투자증권 API의 실시간 시세를 Redis에 캐싱하고 WebSocket과 SSE를 통해 사용자에게 전달합니다.
05. 고가용성·통합 관측 인프라
온프레미스 Active/Standby와 DB 복제, EKS 기반 MSA를 구성하고 Alloy·Prometheus·Grafana·OpenTelemetry로 통합 관측합니다.
